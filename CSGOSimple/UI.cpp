...
void ImGui::Separator(const char* label)
{
	...
	draw_list->AddLine(ImVec2(cursor.x, cursor.y + size.y / 2), ImVec2(pos.x - 2.f, cursor.y + size.y / 2), col);
	draw_list->AddLine(ImVec2(pos.x + size.x + 2.f, cursor.y + size.y / 2), ImVec2(cursor.x + width, cursor.y + size.y / 2), col);
